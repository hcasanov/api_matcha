...
module.exports = {
    post: function(req, res) {
        if (req.body.token == undefined || req.body.from_id == undefined || req.body.to_id == undefined || req.body.message == undefined)
    ...
    get: function(req, res) {
        if (req.body.token == undefined || req.body.from_id == undefined || req.body.to_id == undefined)
            return res.status(400).send('Bad Request');
        pool.connect(function(err, client, done) {
            var id = jwt_decode(req.body.token).id;
            var query = "SELECT token FROM accounts WHERE id = " + id + ";";
